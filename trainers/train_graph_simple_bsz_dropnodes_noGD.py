...
def parse_option():
    ...
    parser.add_argument('--diffusion_method_t', type=str, default='ppr_diffusion')
...
def fractional_matrix_power_pytorch(mat, m):
...
def compute_ppr(affinity, alpha=0.12, self_loop=False):
    a = affinity
    tmp = torch.ones(a.shape[0], a.shape[1]).to(a.device)
    tmp = torch.diag_embed(tmp, offset=0, dim1=1, dim2=2)
    if self_loop:
        a = a + tmp                                          # A^ = A + I_n
    d = torch.diag_embed(torch.sum(a, dim=2) ** (-0.5), offset=0, dim1=1, dim2=2)          # D^ = Sigma A^_ii                         # D^(-1/2)
    at = torch.matmul(torch.matmul(d, a), d)
    res = alpha * torch.inverse((tmp - (1 - alpha) * at))    # A~ = D^(-1/2) x A^ x D^(-1/2)
    del at, tmp, d
    torch.cuda.empty_cache()
    return res                                                                 # a(I_n-(1-a)A~)^-1

def compute_heat(affinity, t=11, self_loop=False):
    a = affinity
    tmp = torch.ones(a.shape[0], a.shape[1]).to(a.device)
    tmp = torch.diag_embed(tmp, offset=0, dim1=1, dim2=2)
    if self_loop:
        a = a + torch.eye(a.shape[0])
    d = torch.diag_embed(torch.sum(a, dim=2), offset=0, dim1=1, dim2=2)
    res = torch.exp(t * (torch.matmul(a, torch.inverse(d)) - 1))
    del tmp, d
    torch.cuda.empty_cache()
    return res

class GraphDiffusion(nn.Module):
    def __init__(self, dropout_rate=0.12, topk=10, alpha=0.12, diffusion_method='ppr_diffusion'):
        super(GraphDiffusion, self).__init__()
        self.dropout_rate = dropout_rate
        self.diffusion_method = diffusion_method
        self.topk = topk
        self.alpha = alpha
        self.register_buffer("identity", torch.ones(1, topk))
        self.identity = torch.diag_embed(self.identity, offset=0, dim1=1, dim2=2)

    def forward(self, sub_graphs):
        affinity = torch.bmm(sub_graphs, sub_graphs.transpose(1,2))
        affinity = F.dropout(affinity, p=self.dropout_rate)
        affinity = torch.min(affinity, affinity.transpose(1,2)) + eps
        if self.diffusion_method == 'ppr_diffusion':
            affinity = self.compute_ppr(affinity)
            res = affinity[:,0:1,:]
            return res
        elif self.diffusion_method == 'heat_diffusion':
            affinity = self.compute_heat(affinity)
            res = affinity[:,0:1,:]
            return res
        elif self.diffusion_method == 'None':
            res = affinity[:,0:1,:]
            return res
        else:
            raise ValueError("Do not support other diffusion methods")

    def compute_ppr(self, affinity):
        a = affinity                               # A^ = A + I_n
        d = torch.diag_embed(torch.sum(a, dim=2) ** (-1), offset=0, dim1=1, dim2=2)          # D^ = Sigma A^_ii                         # D^(-1/2)
        at = torch.matmul(d, a)
        res = self.alpha * torch.inverse(self.identity - (1 - self.alpha) * at)    # A~ = D^(-1/2) x A^ x D^(-1/2)
        return res

    def compute_heat(self, affinity):
        a = affinity
        d = torch.diag_embed(torch.sum(a, dim=2), offset=0, dim1=1, dim2=2)
        res = torch.exp(self.alpha * (torch.matmul(a, torch.inverse(d)) - 1))
        return res

# def Graph_diffusion(sub_graphs, dropout_rate=0.8, diffusion_method='ppr_diffusion'):
#     affinity = torch.bmm(sub_graphs, sub_graphs.transpose(1,2))
#     affinity = F.dropout(affinity, p=dropout_rate)
#     affinity = torch.min(affinity, affinity.transpose(1,2))
#     if diffusion_method == 'ppr_diffusion':
#         affinity = compute_ppr(affinity)
#         res = affinity[:,0:1,:]
#         del affinity
#         torch.cuda.empty_cache()
#         return res
#     elif diffusion_method == 'heat_diffusion':
#         affinity = compute_heat(affinity)
#         res = affinity[:,0:1,:]
#         del affinity
#         torch.cuda.empty_cache()
#         return res
...
class MeanShift(nn.Module):
    def __init__(self, arch, m=0.99, mem_bank_size=128000, topk=5,
                GCN_configs_q=None, GCN_configs_t=None, dropout_rate=0.3,
                diffusion_method_q=None, diffusion_method_t='ppr_diffusion',
                alpha=0.12, node_dropout_rate=0.1):
        ...
        # diffusion layer
        self.diffusion_t = self._build_diffusion(dropout_rate=dropout_rate,
            topk=topk, alpha=alpha, diffusion_method=diffusion_method_t)
    ...
    def data_parallel(self):
        ...
        self.predict_t = torch.nn.DataParallel(self.predict_t)
        self.diffusion_t = torch.nn.DataParallel(self.diffusion_t)
    ...
    @torch.no_grad()
    def _build_diffusion(self, dropout_rate=0.12, topk=10, alpha=0.12, diffusion_method='ppr_diffusion'):
        Diffusion = GraphDiffusion(dropout_rate, topk, alpha, diffusion_method)
        return Diffusion
    ...
    def forward(self, im_q, im_t, labels):
        ...
        bsz = len(im_q)
        adj_query = torch.eye(1, self.topk).reshape(1,1, self.topk).repeat(bsz,1,1).to(im_q.device)
        ##################################################################### # TODO:
        ...
        sampled_graph = targets[nn_index]
        # adj_positive_target = Graph_diffusion(sampled_graph, self.dropout_rate, diffusion_method='ppr_diffusion')
        sampled_graph = F.dropout(sampled_graph, p=self.node_dropout_rate)
        adj_positive_target = self.diffusion_t(sampled_graph)
        positive_target = self.predict_t(current_target, sampled_graph, adj_positive_target)
        positive_target = nn.functional.normalize(positive_target+eps, dim=1)
...
def main():
    ...
    mean_shift = MeanShift(
        ...
        diffusion_method_t=args.diffusion_method_t,

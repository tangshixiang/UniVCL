...
class Image100FolderEx(datasets.ImageFolder):
    ...
    def set_neighboring(self, neighbors):
...
class MeanShift(nn.Module):
    def __init__(self, arch, m=0.99, image_num=128000, mem_bank_size=128000, topk=5,
                    GCN_configs_q=None, GCN_configs_t=None):
        ...
        # setup queue for neighboring list
        self.register_buffer('neighbors', torch.randint(self.image_num, (self.image_num, self.topk)))
        ...
        # initialize the indice queue
        self.register_buffer('indice', -1*torch.ones(self.mem_bank_size).long())
    ...
    @torch.no_grad()
    def _dequeue_and_enqueue(self, targets, labels, indexes):
        ...
        self.indices[ptr:ptr + batch_size] = indexes
        ptr = (ptr + batch_size) % self.mem_bank_size  # move pointer

        self.queue_ptr[0] = ptr

    def forward(self, im_q, im_t, labels, indexes):
        ...
        with torch.no_grad():
            ...
            self._dequeue_and_enqueue(current_target, labels, indexes)
        ...
        self.neighbors[indexes] = nn_index
...
def train(epoch, train_loader, mean_shift, optimizer, opt):
    ...
    for idx, (indices, (im_q, im_t), labels) in enumerate(train_loader):
        ...
        # ===================forward=====================
        loss, purity = mean_shift(im_q=im_q, im_t=im_t, labels=labels, idx=idx)
        loss = loss.mean()
        purity = purity.mean()
